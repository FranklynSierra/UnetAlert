...
import { doc, getDoc, httpsCallable } from "firebase/firestore"; // getDoc/doc si guardas plantilla
import { db, functions } from "../../lib/firebase"; // ajusta la ruta según tu proyecto
...
function makeEmptyActaData(incident) {
  return {
    ...
    jefeNombre: "",
    jefeCedula: "",
    narrativa: "", // texto de los hechos (contenteditable), manual o generado con IA
  };
}
...
export default function ActaIncidenteModal({ isOpen, onClose, persona, cedulaPersona, rolPersona = "el/la trabajador(a)", incidents }) {
  ...
  // Llama a una Cloud Function (ver funciones/generarNarrativaActa.js) que a su
  // vez llama al API de Claude con la data estructurada del incidente. La IA
  // SOLO redacta el párrafo de hechos: nunca inventa nombres, cédulas ni fechas.
  const generarConIA = async (incident) => {
    setAiLoadingId(incident.id);
    try {
      const call = httpsCallableFn(functions, "generarNarrativaActa");
      const { data } = await call({
        tipo: incident.tipo || "",
        descripcion: incident.descripcion || "",
        gravedad: incident.gravedad || "",
        lugar: actaData[incident.id]?.lugar || incident.lugar || "",
        persona,
      });
      updateField(incident.id, "narrativa", data.narrativa);
    } catch (err) {
      console.error("Error generando narrativa con IA:", err);
      alert("No se pudo generar la redacción con IA. Puedes escribirla manualmente.");
    } finally {
      setAiLoadingId(null);
    }
  };
            ...
            {previewImages.length === 0 ? (
              <>
                <p className="text-muted small">
                  Selecciona los incidentes a incluir. Completa testigos, lugar y hora, y usa
                  "Redactar con IA" para generar el párrafo de hechos automáticamente (revisable antes de imprimir).
                </p>
                              ...
                              <div className="row g-2 mt-3">
                                ...
                                <div className="col-12">
                                  <div className="d-flex justify-content-between align-items-center mb-1">
                                    <label className="form-label small mb-0">Narrativa de los hechos</label>
                                    <button
                                      className="btn btn-sm btn-outline-primary"
                                      disabled={aiLoadingId === incident.id}
                                      onClick={() => generarConIA(incident)}
                                    >
                                      {aiLoadingId === incident.id ? "Redactando..." : "Redactar con IA"}
                                    </button>
                                  </div>
                                  <textarea
                                    ...
                                    placeholder="Ej: Que el trabajador no se presentó en su sitio de trabajo..."
                                  />
                                </div>
